...
SAMPLE_NEOMOD3_EXTRAS = pathlib.Path(__file__).parent / "data" / "neomod3_extras_sample.txt"
...
class Test_NEOMOD3:
    ...
    def test_load_table_extras(self, tmp_path):
        # Copy the extended-format sample file to a temporary location for testing
        test_file = tmp_path / "neomod3_extras_sample.txt"
        shutil.copy(SAMPLE_NEOMOD3_EXTRAS, test_file)

        # Create an instance of NEOMOD3 with the temporary file
        neomod3 = NEOMOD3(data_root=tmp_path, filename="neomod3_extras_sample.txt")

        # Load the table and check its contents
        table = neomod3.neo_table

        # Check that the table has the expected columns, including the extras
        expected_columns = [
            "H", "a", "e", "inc", "diam", "pV",
            "Omega", "argPeri", "meanAnomaly", "epoch", "sed_filename",
        ]
        assert all(col in table.colnames for col in expected_columns)
